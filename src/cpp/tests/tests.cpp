...
TEST(Svar,Thread){
    auto doubleThread=[](){
        while(!jvar.get<bool>("shouldstop",false)){
            double d=jvar.get<double>("thread.Double",100);
            jvar.set<double>("thread.Double",d++);
            ...
            Svar v=jvar.get("thread.Double",100);
    ...
    auto intThread=[](){
        while(!jvar.get<bool>("shouldstop",false)){
            ...
            int i=jvar.get<int>("thread.Double",10);
            jvar.set<int>("thread.Double",i);

            GSLAM::Rate::sleep(1e-4);
        }
    };
    std::vector<std::thread> threads;
    for(int i=0;i<jvar.GetInt("doubleThreads",4);i++) threads.push_back(std::thread(doubleThread));
    for(int i=0;i<jvar.GetInt("intThreads",4);i++) threads.push_back(std::thread(intThread));

    GSLAM::Rate::sleep(1);
    jvar.set("shouldstop",true);
...
REGISTER_SVAR_MODULE(tests){
    jvar["apps"]["tests"]={tests,"Use 'svar tests' to perform module testing"};
}
